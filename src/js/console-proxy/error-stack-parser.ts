...
export class StackFrame {
  ...
  toString() {
    ...
    const columnNumber = this.col || "";
    if (this.func)
      return `${this.func} @(${this.file}:${lineNumber}:${columnNumber})`;
    return `(anonymous) @(${this.file}:${lineNumber}:${columnNumber})`;
  }
}
  ...
  parseV8OrIE: function(error: { stack: string }) {
    ...
    return filtered.map(function(line) {
      ...
      return new StackFrame({
        func: functionName,
        file: fileName,
        line: parseFloat(locationParts[1]),
        col: parseFloat(locationParts[2]),
      });
    }, this);
  },
